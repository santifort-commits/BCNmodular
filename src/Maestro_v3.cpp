...
// Widget per indicador de voltatge al connector
struct VoltagePort : PJ301MPort {
    float voltage = 0.f;
    bool isOutput = false;

    void draw(const DrawArgs& args) override {
        PJ301MPort::draw(args);
        // Cercle indicador de voltatge al centre
        float v = rack::math::clamp(std::abs(voltage) / 10.f, 0.f, 1.f);
        NVGcolor col = nvgRGBf(0.f, v, 0.f);
...
struct Maestro : Module {
    ...
    int minVoices = 0; // mínim de veus actives (menu contextual)
    ...
    Maestro() {
        ...
        // RESTRICT: esquerra=determinístic (0), dreta=aleatori (1)
    ...
    void evaluate(int numChannels, float densityTarget, float randomness) {
        ...
        if (randomness <= 0.001f) {
            // Totalment determinístic
            targetVoices = (int)rack::math::clamp(std::round(densityTarget), 0.f, (float)numChannels);
        } else {
            // Dispersió gaussiana: randomness=0 → exacte, randomness=1 → molt aleatori
    ...
    void process(const ProcessArgs& args) override {
        ...
        for (int i = 0; i < MAX_CH; i++) {
            ...
            if (channelOpen[i] && !inFade[i]) {
                ...
                lights[lightBase].setBrightness(fadeLevel[i]);
                lights[lightBase + 1].setBrightness(0.f);
            } else {
