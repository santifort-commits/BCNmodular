...
struct Maestro : Module {
    ...
    int minVoices = 0;
    int beatsPerBar = 4;  // configurable via menu contextual
    float chVoltage[MAX_CH]; // per als indicadors de voltatge
    ...
    // Converteix 0-4 a compassos: 1, 2, 4, 8, 16
    ...
    void process(const ProcessArgs& args) override {
        ...
        for (int i = 0; i < MAX_CH; i++) {
            ...
            chVoltage[i] = inSignal * fadeLevel[i];
            outputs[CH_OUTPUT_1 + i].setVoltage(chVoltage[i]);

            // LEDs: verd=obert, groc=fade, apagat=tancat
            int lightBase = i * 2;
            if (channelOpen[i] && !inFade[i]) {
                lights[lightBase].setBrightness(1.f);      // verd
                lights[lightBase + 1].setBrightness(0.f);
                activeCount++;
            } else if (inFade[i] && fadeMode) {
                lights[lightBase].setBrightness(1.f);      // verd + vermell = groc
...
struct MaestroWidget : ModuleWidget {
    // Punters als ports de sortida per actualitzar voltatge
    VoltageOutputPort* chOutPorts[6] = {};
