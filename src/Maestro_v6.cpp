...
struct Maestro : Module {
    ...
    float defaultInputVoltage = 1.f;  // 1V o 10V
    bool activeOutProportional = true; // proporcional o absolut
    ...
    void evaluate(int numChannels, float densityTarget, float randomness) {
        // Clamp density entre minVoices i numChannels
    ...
    void process(const ProcessArgs& args) override {
        ...
        if (activeOutProportional) {
            // Proporcional al nombre de canals actius (Active Tracks)
            activeCV = (numChannels > 0) ?
                       (float)activeCount / (float)numChannels * 10.f : 0.f;
        } else {
            // Absolut: 1.666V per canal actiu
...
struct MaestroWidget : ModuleWidget {
    ...
    void appendContextMenu(Menu* menu) override {
        ...
        menu->addChild(new MenuSeparator);
        menu->addChild(createMenuLabel("Default input voltage"));
        menu->addChild(createCheckMenuItem(
            "1V (gate/trigger)",
            "",
        ...
        menu->addChild(new MenuSeparator);
        menu->addChild(createMenuLabel("Active output CV mode"));
        menu->addChild(createCheckMenuItem(
            "Proportional to active tracks",
            "",
            [=]() { return module->activeOutProportional; },
            [=]() { module->activeOutProportional = true; }
        ));
        menu->addChild(createCheckMenuItem(
            "Absolute (1.66V per voice)",
            "",
            [=]() { return !module->activeOutProportional; },
